...
def now(request):
    ...
    avg_per_active = round((row.messages or 0) / active_authors, 2) if active_authors else 0.0
    ...
    return JsonResponse({
        ...
        'joins': row.joins,
        'leaves': row.leaves,
        'diff': (row.joins or 0) - (row.leaves or 0),
...
def export_xlsx(request):
    ...
    for r in Daily.objects.order_by('date').values(
        ...
        ws.append([
            ...
            r['voice_seconds'] or 0, round((r['voice_seconds'] or 0) / 3600, 2),
            authors, avg
        ])
